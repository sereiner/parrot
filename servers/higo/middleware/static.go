...
// root directory.
func Static(root string) higo.MiddlewareFunc {
...
// See `Static()`.
func StaticWithConfig(config StaticConfig) higo.MiddlewareFunc {
	// Defaults
	if config.Root == "" {
			...
			if err != nil {
				if os.IsNotExist(err) {
					if err = next(c); err != nil {
						if he, ok := err.(*higo.HTTPError); ok {
...
func listDir(t *template.Template, name string, res *higo.Response) (err error) {
	...
	res.Header().Set(higo.HeaderContentType, higo.MIMETextHTMLCharsetUTF8)
	data := struct {
		Name  string
		Files []interface{}
	}{
