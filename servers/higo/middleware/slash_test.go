...
func TestAddTrailingSlash(t *testing.T) {
	...
	h := AddTrailingSlash()(func(c higo.Context) error {
		return nil
	})
	is.NoError(h(c))
	...
	is.Equal("/add-slash/?key=value", rec.Header().Get(higo.HeaderLocation))
}
...
func TestRemoveTrailingSlash(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	c := e.NewContext(req, rec)
	h := RemoveTrailingSlash()(func(c higo.Context) error {
		return nil
	})
	is.NoError(h(c))
	...
	is.NoError(h(c))
	is.Equal(http.StatusMovedPermanently, rec.Code)
	is.Equal("/remove-slash?key=value", rec.Header().Get(higo.HeaderLocation))
