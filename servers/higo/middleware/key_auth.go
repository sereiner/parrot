...
type (
	...
	KeyAuthValidator func(string, higo.Context) (bool, error)

	keyExtractor func(higo.Context) (string, error)
)
...
var (
	...
	DefaultKeyAuthConfig = KeyAuthConfig{
		...
		KeyLookup:  "header:" + higo.HeaderAuthorization,
		AuthScheme: "Bearer",
	}
)
...
func KeyAuth(fn KeyAuthValidator) higo.MiddlewareFunc {
...
// See `KeyAuth()`.
func KeyAuthWithConfig(config KeyAuthConfig) higo.MiddlewareFunc {
	// Defaults
	...
	if config.Validator == nil {
		panic("higo: key-auth middleware requires a validator function")
	}
	...
	return func(next higo.HandlerFunc) higo.HandlerFunc {
		return func(c higo.Context) error {
			...
			key, err := extractor(c)
			if err != nil {
				return higo.NewHTTPError(http.StatusBadRequest, err.Error())
			}
...
func keyFromHeader(header string, authScheme string) keyExtractor {
	return func(c higo.Context) (string, error) {
		auth := c.Request().Header.Get(header)
		if auth == "" {
			return "", errors.New("missing key in request header")
		}
		if header == higo.HeaderAuthorization {
			l := len(authScheme)
