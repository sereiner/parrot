...
// G, T or P.
func BodyLimit(limit string) higo.MiddlewareFunc {
...
// See: `BodyLimit()`.
func BodyLimitWithConfig(config BodyLimitConfig) higo.MiddlewareFunc {
	// Defaults
	...
	if err != nil {
		panic(fmt.Errorf("higo: invalid body-limit=%s", config.Limit))
	}
	config.limit = limit
	...
	return func(next higo.HandlerFunc) higo.HandlerFunc {
		return func(c higo.Context) error {
			...
			if req.ContentLength > config.limit {
				return higo.ErrStatusRequestEntityTooLarge
			}
...
func (r *limitedReader) Read(b []byte) (n int, err error) {
	...
	r.read += int64(n)
	if r.read > r.limit {
		return n, higo.ErrStatusRequestEntityTooLarge
	}
	return
}
...
func (r *limitedReader) Reset(reader io.ReadCloser, context higo.Context) {
	r.reader = reader
	r.context = context
	r.read = 0
}
