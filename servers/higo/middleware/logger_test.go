...
func TestLogger(t *testing.T) {
	...
	h := Logger()(func(c higo.Context) error {
...
func TestLoggerIPAddress(t *testing.T) {
	...
	// With X-Real-IP
	req.Header.Add(higo.HeaderXRealIP, ip)
	h(c)
	assert.Contains(t, ip, buf.String())

	// With X-Forwarded-For
	buf.Reset()
	req.Header.Del(higo.HeaderXRealIP)
	req.Header.Add(higo.HeaderXForwardedFor, ip)
	h(c)
	assert.Contains(t, ip, buf.String())
...
func TestLoggerTemplate(t *testing.T) {
	...
	req.RequestURI = "/"
	req.Header.Add(higo.HeaderXRealIP, "127.0.0.1")
	req.Header.Add("Referer", "google.com")
	req.Header.Add("User-Agent", "higo-tests-agent")
