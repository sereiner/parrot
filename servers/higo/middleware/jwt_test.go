...
func TestJWTRace(t *testing.T) {
	...
	makeReq := func(token string) higo.Context {
		req := httptest.NewRequest(http.MethodGet, "/", nil)
		res := httptest.NewRecorder()
		req.Header.Set(higo.HeaderAuthorization, DefaultJWTConfig.AuthScheme+" "+token)
...
func TestJWT(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		res := httptest.NewRecorder()
		req.Header.Set(higo.HeaderAuthorization, tc.hdrAuth)
		req.Header.Set(higo.HeaderCookie, tc.hdrCookie)
		c := e.NewContext(req, res)
