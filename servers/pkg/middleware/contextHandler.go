...
func getUUID(c *dispatcher.Context) string {
	sid, ok := c.Request.GetHeader()["__parrot_sid_"]
	if !ok || sid == "" {
...
func setUUID(c *dispatcher.Context, id string) {
	c.Request.GetHeader()["__parrot_sid_"] = id
}
...
func ContextHandler(exhandler interface{}, name string, engine string, service string, mSetting map[string]string, ext map[string]interface{}) dispatcher.HandlerFunc {
	...
	return func(c *dispatcher.Context) {
		//处理输入参数
		ctx := context.GetContext(nil,exhandler, name, engine, service, exhandler.(context.IContainer), makeQueyStringData(c), makeFormData(c), makeParamsData(c), makeSettingData(c, mSetting), makeExtData(c, ext), getLogger(c))
