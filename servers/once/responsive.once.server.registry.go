...
func (s *OnceResponsiveServer) watchMasterChange(root, path string) error {
	...
	servers.Tracef(s.Infof, "%s", types.DecodeString(s.master, true, "master once server", "slave once server"))
	...
	go func() {
		for {
			...
			case cldWatcher := <-children:
				...
				if master != s.master {
					servers.Tracef(s.Infof, "%s", types.DecodeString(master, true, "master once server", "slave once server"))
